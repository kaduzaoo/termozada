      ...
      <div key={`row-${rowIndex}`} className="flex gap-2 justify-center mb-2">
        {Array.from({ length: wordLength }).map((_, index) => {
          ...
          return (
            <div
              key={index}
              onClick={() => isCurrentGuess onClick={() => isCurrentGuess && onPositionClick(index)}onClick={() => isCurrentGuess && onPositionClick(index)} !statuses[index] onClick={() => isCurrentGuess && onPositionClick(index)}onClick={() => isCurrentGuess && onPositionClick(index)} onPositionClick(index)}
